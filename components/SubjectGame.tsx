...
export default function SubjectGame({ subjectId, startFloor, playerName, onFloorCleared, onBack }: Props) {
  ...
  if (phase === "intro") {
    return (
      <div className={`min-h-screen bg-gradient-to-b ${subject.gradient.replace("from-", "from-").replace("to-", "to-")} flex flex-col items-center justify-center p-8 gap-6`}>
        <div className="text-8xl animate-bounce select-none">{level.emoji}</div>
        <div className="text-center">
          <p className="text-white/70 text-base font-bold">{subject.emoji} {subject.name}</p>
          <h1 className="text-white font-black text-3xl mt-1">{level.name}</h1>
          <p className="text-white/60 text-sm mt-2">{level.grade}</p>
          <p className="text-white/80 text-base mt-2">{level.desc}</p>
        </div>
        <div className="bg-black/20 rounded-3xl px-8 py-3">
          <p className="text-white font-bold text-lg">Etage {floor} / 10</p>
        </div>
  ...
  if (phase === "complete") {
    ...
    return (
      <div className={`min-h-screen bg-gradient-to-b ${subject.gradient} flex flex-col items-center justify-center p-8 gap-5`}>
        <div className="text-8xl select-none animate-bounce">{star}</div>
        <h1 className="text-white font-black text-3xl text-center">Etage {floor} klaret!</h1>
        <div className="bg-black/30 rounded-3xl p-5 w-full max-w-xs space-y-3">
    ...
    <div className={`min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex flex-col`}>
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-white font-black">{level.name}</p>
        </div>
        <div className="text-right">
          <p className="text-yellow-400 font-black text-lg">{qIdx + 1}/{totalQ}</p>
          {streak >= 2 && <p className="text-orange-400 text-xs font-bold">🔥 {streak} stribe!</p>}
        </div>
      </div>

      {/* Progress bar */}
      <div className="h-2 bg-white/10 mx-4 rounded-full overflow-hidden mb-6">
        <div className={`h-full bg-gradient-to-r ${subject.gradient} rounded-full transition-all duration-500`}
          style={{ width: `${progress * 100}%` }} />
      </div>

      {/* Question card */}
      <div className="mx-4 bg-gradient-to-br from-slate-800 to-slate-700 rounded-3xl p-6 mb-6 shadow-2xl border border-white/10">
        <div className="text-5xl text-center mb-4 select-none">{level.emoji}</div>
        <p className="text-white font-bold text-xl text-center leading-tight">{currentQ.q}</p>
      </div>
        ...
        {currentQ.opts.map((opt, i) => {
          ...
          return (
            <button
              ...
              className={`border-2 rounded-2xl py-5 px-3 font-bold text-base text-center transition-all select-none ${style}`}
            >
              {opt}
            </button>
          );
      ...
      {selected !== null && phase === "feedback" && (
        <div className={`mx-4 mt-5 rounded-2xl p-4 text-center font-black text-xl ${selected === currentQ.a ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
          {selected === currentQ.a
