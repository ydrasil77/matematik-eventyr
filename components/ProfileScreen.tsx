      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-3 flex flex-col gap-3">
        {profiles.map((p) => (
          <div key={p.id} className="relative flex-shrink-0">
            {confirmDelete === p.id && (
              <div className="absolute inset-0 z-10 bg-red-500 rounded-2xl flex items-center justify-center gap-3 px-4">
                <span className="text-white font-bold text-lg flex-1">Slet {p.name}?</span>
                <button onPointerDown={() => { onDelete(p.id); setConfirmDelete(null); }}
                  className="bg-white text-red-600 font-black px-4 py-3 rounded-xl text-lg active:scale-95 select-none">
                  Ja ✓
                </button>
                <button onPointerDown={() => setConfirmDelete(null)}
                  className="bg-white/30 text-white font-bold px-4 py-3 rounded-xl text-lg active:scale-95 select-none">
                  Nej ✗
                </button>
              </div>
            )}
